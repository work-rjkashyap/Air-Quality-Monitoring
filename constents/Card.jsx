...
import getAQILevel from '../utils/getaqileve';
...
function Card({CityName,AQI,onClick}) {
  ...
  return (
    <div className="d-flex flex-column" onClick={ResizeCard}  aria-hidden>
    ...
    <div className="flex justify-between">
      ...
      <span className={`font-bold text-[${status.color}]`}>{status && status.name}</span>
      <div style={{width:'25px',height:'25px',borderRadius:'50%'}} className={`${(status && status.className) || ''} `}>&emsp;</div>
    </div>
